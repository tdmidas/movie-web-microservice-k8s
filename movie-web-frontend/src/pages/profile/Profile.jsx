...
const Profile = () => {
  ...
  useEffect(() => {
    const getInfo = async () => {
      const res = await axios.get(`http://${process.env.REACT_APP_GATEWAY_HOST}:${process.env.REACT_APP_GATEWAY_PORT}/service1/api/users/find/${user.email}`, {
        headers: {
  ...
  const upProfile = async () => {
    try {
      const res = await axios.put(`http://${process.env.REACT_APP_GATEWAY_HOST}:${process.env.REACT_APP_GATEWAY_PORT}/service1/api/users/change/${user.email}`, info , {
        headers: {
          token: "Bearer " + JSON.parse(localStorage.getItem("user")).accessToken
        }
      });
