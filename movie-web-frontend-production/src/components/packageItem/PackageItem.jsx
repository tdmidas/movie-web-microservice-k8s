...
const PackageItem = props => {
    ...
    useEffect(() => {
        const getPackage = async () => {
            const res = await axios.get(`${process.env.REACT_APP_GATEWAY_BACKEND_URL}/service3/api/bills/getPackage/${props.id}`, {
                headers: {
    ...
    useEffect(() => {
        const getInfoTransfer = async() => {
            const res = await axios.post(`${process.env.REACT_APP_GATEWAY_BACKEND_URL}/service3/api/bills/payment`, {
                    email: user.email,
