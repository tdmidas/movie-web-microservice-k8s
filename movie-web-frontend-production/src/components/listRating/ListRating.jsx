...
const ListRating = () => {
    ...
    useEffect(() => {
        const getRating = async () => {
            try {
                const res = await axios.get(`${process.env.REACT_APP_GATEWAY_BACKEND_URL}/service2/api/ratings/getAll`, {
                    headers: {
        ...
        const getUser = async () => {
            try {
                const res = await axios.get(`${process.env.REACT_APP_GATEWAY_BACKEND_URL}/service2/api/movies/getInfo`, {
                    headers: {
                        token: "Bearer "+JSON.parse(localStorage.getItem("user")).accessToken
                    }
                })
    ...
    useEffect(() => {
        const findHighestRatingMovies = async () => {
            try {
                const response = await axios.post(`${process.env.REACT_APP_GATEWAY_BACKEND_URL}/service2/api/movies/popular`, {
                    ratings: ratings,
                    movies: movies
                }, {
                    headers: {
